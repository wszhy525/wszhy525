...
int Serial::receive()
{
    ...
    while (readCount < int(sizeof(FeedBackFrame)))
	{
        ...
        if ((chrono::duration_cast<std::chrono::milliseconds>(t2 - t1)).count() > 100) // Time limit is 10ms
        {
            if(_en_debug)
            {
...
int Serial::receive_longtime()
{
    ...
    {
        auto t2 = std::chrono::high_resolution_clock::now();
        if ((chrono::duration_cast<std::chrono::milliseconds>(t2 - t1)).count() > 500) // Time limit is 400ms
        {
            if(_en_debug)
            {
                cout << "\tReceiving time out. " << sizeof(FeedBackFrame) - readCount
