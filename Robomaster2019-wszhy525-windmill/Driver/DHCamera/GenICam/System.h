...
public:
	...
	/// \~chinese
	/// \brief 发现指定接口类型可达的设备，接口类型可任意组合
	/// \param [out] vCameraPtrList 指定接口类型所有在线设备对象列表,
	///\ 1、当interfaceType = interfaceTypeAll  时，发现所有接口下的在线设备
	///\ 2、当interfaceType = interfaceTypeGige 时，发现所有 GigE 网口下的在线设备
	///\ 3、当interfaceType = interfaceTypeUsb3 时，发现所有 USB3 接口下的在线设备
	///\ 4、当interfaceType = interfaceTypeCL   时，发现所有 CameraLink 接口下的在线设备
	///\ 5、该接口下的interfaceType支持任意接口类型的组合,如，若发现所有GigE网口和USB3接口下的在线设备时,
	///\  可将interfaceType设置为 interfaceType = interfaceTypeGige | interfaceTypeUsb3,其它接口类型组合以此类推
	/// \param [in] interfaceType 待发现的接口类型,类型可任意组合,如 interfaceTypeGige | interfaceTypeUsb3
	/// \return 是否成功
	/// \~english
	/// \brief discover reachable camera which has specified interface type, support any combination of interface types 
	/// \param [out] vCameraPtrList camera instance list of specified interface type
	///\ 1、when interfaceType = interfaceTypeAll, discover cameras in all interface types
	///\ 2、when interfaceType = interfaceTypeGige, find cameras in GigE interface 
	///\ 3、when interfaceType = interfaceTypeUsb3, find cameras in USB3 interface
	///\ 4、when interfaceType = interfaceTypeCL, find cameras in CameraLink interface
	///\ 5、interfaceType supports any interface type combination. For example, if you want to find all GigE and USB3 cameras,
	///\ you can set interfaceType as interfaceType = interfaceTypeGige | interfaceTypeUsb3.
	/// \param [in] interfaceType the interface type you want to find, support any interface type combination, sucn as interfaceTypeGige | interfaceTypeUsb3
	/// \return success:true, fail:false
	bool discovery(Infra::TVector<CIntegratedCameraPtr>& vCameraPtrList, int32_t interfaceType = interfaceTypeAll);
