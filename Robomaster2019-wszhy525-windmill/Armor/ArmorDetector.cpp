...
void ArmorDetector::init(const ArmorParam &armorParam)
{
 ...
 //   _cudaDilateFilter = cuda::createMorphologyFilter( MORPH_DILATE, CV_8U, element );
...
//        cuda::GpuMat binBrightImg1, grayImg1;
//        binBrightImg1.upload(_roiImg);

//        cuda::cvtColor(binBrightImg1, grayImg1, COLOR_BGR2GRAY,1);

//        cuda::threshold(grayImg1, binBrightImg1,_param.brightness_threshold, 255, cv::THRESH_BINARY);

//        cv::Mat element = cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(3, 3));

//        _cudaDilateFilter->apply(binBrightImg1, binBrightImg1);


//        binBrightImg1.download(binBrightImg);
//        grayImg1.download(_grayImg);
...
//                _cudaGrayImg.download(_grayImg);
